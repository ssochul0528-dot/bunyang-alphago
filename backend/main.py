...
def generate_lms_variants(req: AnalysisRequest, gap_percent: float):
    v1 = f"(광고) 💎 {req.field_name} | {req.address} 선착순 분양\n\n주변 시세 대비 {int(abs(gap_percent))}% 저렴! 계약금 {req.down_payment}로 입주까지.\n\n▶ 대표전화: 1600-1234"
    return [v1, v1, v1]

def generate_channel_talk_variants(req: AnalysisRequest, gap_percent: float):
    return [f"🔥 {req.field_name} 조건 파격변경 🔥", f"🚨 마감임박! 잔여세대 확인", f"💎 {req.field_name} 프리미엄"]
...
async def analyze_field(request: AnalysisRequest):
    ...
    return AnalysisResponse(
        score=88, score_breakdown=ScoreBreakdown(price_score=40, location_score=25, benefit_score=23, total_score=88),
        market_diagnosis="최상의 공급 조건입니다.", ad_recommendation="릴스 광고 추천",
        media_mix=[MediaRecommendation(media="메타", feature="타겟팅", reason="효과적", strategy_example="릴스")],
        copywriting="지금 기회 잡으세요", price_data=[], radar_data=[], market_gap_percent=gap_percent,
        target_audience=[], target_persona="", competitors=[], roi_forecast=ROIForecast(expected_leads=100, expected_cpl=30000, conversion_rate=5),
...
if __name__ == "__main__":
    import uvicorn
    # 🚨 이 부분이 핵심: Railway가 주는 포트번호를 무조건 따름
