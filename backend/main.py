...
async def analyze_site(request: Optional[AnalyzeRequest] = None):
    ...
    try:
        ...
        3. media_mix: 고민사항인 [{main_concern}]을 즉각 해결할 수 있는 매체 3종을 선정하십시오.
           - 필수 조건: 3종의 매체 중 **'호갱노노 채널톡'**과 **'LMS(문자 마케팅)'**는 어떤 고민 선택 시에도 반드시 포함시키십시오.
           - 나머지 1종은 고민사항 해결에 가장 특화된 매체를 선정하십시오. (전문가 팁: 'DB 수량 부족' 시 인스타그램/당근마켓 등 리드 생성 매체, '방문 예약 저조' 시 유튜브 등 고관여 매체)
           총 3종의 매체 각각에 대해 '운영 전략'과 '소재 제작 포인트'를 전문가 수준으로 서술하십시오.
